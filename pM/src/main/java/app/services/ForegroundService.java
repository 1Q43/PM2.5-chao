...
import app.bandmodule.MonitorDisplayActivity;
...
//        }
        ...
        new Thread(new Runnable() {
            ...
            // upload heartrates to internet
            DataServiceUtil dataServiceUtil = DataServiceUtil.getInstance(getApplicationContext());
...
//                Log.v("Crysa_location", "getLatitude:" + state.getLatitude() + "getLongtitude:" + state.getLongtitude());
                ...
                if(dataServiceUtil.getUpload() == false){
                    if(dataServiceUtil.isForeground(getApplicationContext(), "app.bandmodule.MonitorDisplayActivity"))
                    {
                        Log.e("BandInfo-","在前台");
                    }else{
                        Log.e("BandInfo-", "不在前台");
